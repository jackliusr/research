...
- **Digital bank licenses:** MAS awarded digital full-bank licenses to **Trust** (Standard Chartered-led consortium) and **GXS** (Grab/Singtel), and digital wholesale-bank licenses to **MariBank** (Sea/Shopee) and **ANEXT** (Ant Group) — a deliberate regulatory experiment in tech-native banking (see the digital bank discussion in [core_banking_systems_guide.md](core_banking_systems_guide.md)). These institutions are the licensed beachhead of the programmable model in SG (GXS/MariBank moving into SME/wholesale products).
