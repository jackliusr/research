...
> **Banking aside.** A Cymbal Bank policy library is measured in thousands of pages; even today's 1M-token windows cannot hold it all. The pre-RAG constraint — *weights-only knowledge* — is why the bank's first generative-AI pilots were all retrieval projects. The timeline below is the intellectual history of that decision.
