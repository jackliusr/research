...
Selection rules for banking: (1) test on *your* corpus and QA set, not MTEB — domain jargon changes rankings; (2) prefer multilingual when the corpus mixes EN/FR/DE (Cymbal Bank footprint); (3) check the embedding model's context window against your chunk size; (4) for on-prem, choose a self-hostable family (BGE/GTE/E5) and validate GPU sizing; (5) treat the embedding model as a governed model artifact (SR 11-7, §15.3) — pin and revalidate versions.
...
For Cymbal Bank-style environments (regulated, document-heavy, hybrid-retrieval-needing, data-sovereignty-constrained), the recommended reference stack composes the categories above:
