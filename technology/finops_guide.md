...
## 19. Worked Example: The Meridian Bank FinOps Implementation

> **Scenario — the familiar context:** Meridian Bank Asia (the recurring fictional persona in this series — a Cymbal Bank with a Singapore hub) runs a two-year cloud migration of its payments and risk platforms. The cloud bill has grown from S$400k/month to S$2.1M/month as workloads moved; the CFO's office and the CTO's office are blaming each other. The bank commissions a FinOps programme. This worked example designs it end-to-end, following the framework of §§2-8 of this guide.

### 19.1 The Starting Point: The Cost Crisis

| Metric | Before FinOps | After 12 months |
|---|---|---|
| Monthly cloud spend | S$2.1M (all providers, untagged) | S$1.62M (tagged, allocated) |
| Share of spend with cost-allocation tags | ~5% | 98% |
| Teams that could see their own costs | 1 (the CCoE) | 14 (all product teams) |
| Untagged "zombie" resources | ~S$380k/month | ~S$40k/month |
| Unit cost per payment transaction | S$0.0041 | S$0.0022 |

### 19.2 The Design (Three Phases)

**Phase 1 — Inform (months 1-3).** Standing up the FinOps CoE (the §13 operating model): two finance analysts, one cloud engineer, one platform owner, sponsored by the COO. The tagging taxonomy (§7) is designed around the bank's product lines — payments, trade finance, lending, risk — plus the environment (prod/non-prod) and the cost-centre. The four providers' native tools (§14.2 of cloud_providers_guide.md) are wired to a single reporting pane; showback starts immediately (the §7 showback-vs-chargeback ladder).

**Phase 2 — Optimize (months 4-9).** The three optimization levers (§8) applied in order: (1) **right-sizing** — 31% of compute instances found over-provisioned; (2) **commitment discounts** — 60% of the steady-state compute moved to Savings Plans/RIs/CUDs (the §6 pricing-model matrix); (3) **spot/on-demand mix** for the batch risk simulations (the §8.5 spot section). The first monthly unit-economics report (the §15 metrics) lands: **cost per payment transaction** as the board-level metric, computed as total payments-cloud-cost ÷ payments volume.

**Phase 3 — Operate (months 10-24).** Chargeback replaces showback for the two largest product lines (§7.3); the FinOps Review cadence (the §3 Operate phase) becomes a monthly business ritual with the §5 personas; the anomaly-detection alerts (§15.3) cut the surprise-bill incidents from four per quarter to zero in the second year; the GreenOps overlay (§17) ties the compute reduction to the bank's sustainability disclosures.

### 19.3 The Worked Unit Economics (The Board Table)

The payments platform processes 12M transactions/month on cloud. Monthly costs before optimization: compute S$28,000, storage S$9,400, network/egress S$11,600, managed services S$600 → **S$49,600/month → S$0.00413 per transaction**. After the Phase-2 levers: compute S$15,200 (right-sizing −31% then commitments −40% on the remainder), storage S$7,900, network S$8,100, managed S$600 → **S$31,800/month → S$0.00265 per transaction** — a **36% unit-cost reduction** without touching the application code. The worked lesson: unit economics (§15.4) convert "the cloud bill went up" from a finance-vs-tech argument into a board-readable productivity metric.

### 19.4 The Savings Ledger and the Lessons

| Lever | Annualised saving | Effort | Risk |
|---|---|---|---|
| Right-sizing | S$1.1M | Low (tool-assisted) | Low — needs the workload profiling |
| Commitment discounts | S$2.4M | Medium (the finance approval) | Medium — the lock-in vs the flexibility trade |
| Spot for batch | S$0.4M | Low | Low — the interruptible workloads only |
| Zombie cleanup | S$1.2M | Low | Low — the resource ownership first |
| **Total** | **S$5.1M/yr** (≈20% of the run-rate) | — | — |

**The six lessons:** (1) tagging is a governance decision, not a technical one — the COO sponsorship made it stick; (2) the unit metric beats the aggregate — cost-per-transaction survived the CFO review, "the bill" did not; (3) commitments need the finance sign-off early — the procurement cycle is the critical path; (4) showback before chargeback — the chargeback demands the budget authority that comes later; (5) the tooling is the enabler, the culture is the change (the §14 change-management section); (6) the savings recur only with the Operate-phase cadence — FinOps is a loop (§3), not a project.

> **Cross-ref:** the §18 Implementation Roadmap is the generic sequence; this worked example is its concrete, numbered application. The investment case for the CoE itself (the headcount vs the S$5.1M) is exactly the [business_case_development_guide.md](../management/business_case_development_guide.md) discipline.
...
## Appendix D: Verification and Claims Status

| Claim | Status | Notes |
|---|---|---|
| FinOps = cloud financial management combining culture, practice, tooling | ✅ | FinOps Foundation framing (finops.org); consistent across §§1-2 |
| The three-phase lifecycle Inform → Optimize → Operate | ✅ | FinOps Foundation canonical; the §3 table |
| Crawl → Walk → Run maturity model | ✅ | FinOps Foundation maturity model; the §4 table |
| 2026 State of FinOps: 90% manage SaaS alongside cloud costs (65% in 2025) | ⚠ | The survey figures as reported in the source consulted; the survey methodology and the sample vary year to year |
| Native cost-management tool feature tables (AWS/Azure/GCP/Alibaba) | ✅/⚠ | The feature presence verified against the providers' documentation at time of writing; the specific SKU features evolve quarterly |
| Savings Plans / RI / CUD mechanics | ✅ | AWS/Azure/GCP pricing pages; the §6 table |
| Tagging/chargeback/showback practice | ✅ | The FinOps Foundation + practitioner literature; the §7 table |
| Kubernetes cost tooling (Kubecost, OpenCost, InfraCost) | ✅ | The vendors' documentation; the §12 table |
| FinOps in banking (§11) | ✅/⚠ | The regulatory-cost angle (BCBS 239 cost data) cross-ref'd; the bank-specific figures are illustrative |
| Worked-example numbers (§19) | ⚠ | Fictional Meridian Bank scenario — the figures are illustrative of the method, not a real bank's data |
| Certification programme (FinOps Certified Practitioner) | ✅ | learn.finops.org; the §16 section |

**Verification method:** live web verification at time of writing (2026-07) against the FinOps Foundation, the providers' pricing/management pages, and the vendor documentation; the unverifiable items flagged ⚠ above are either time-varying survey statistics or illustrative worked numbers. No fabricated facts; the flagged items carry the ⚠ marker inline and here.
