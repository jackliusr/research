...
> **Banking reality:** For Crédit Agricole CIB, client-relationship data (names, mandates, facility terms) is the crown jewel. The question "does this prompt contain client data?" is unanswerable at scale for free-text workloads — so the safe default is that it *might*, and the architecture must therefore keep inference local.
...
| **Mistral** | 7B, 8x22B MoE | Apache 2.0 | Efficient, strong Europe/FR language coverage (relevant for CACIB); 8x22B MoE gives 70B-class quality at lower active-param cost. |
