...
- Naming rules: 3–63 characters, lowercase letters, numbers, dots, and hyphens; must begin and end with a letter or number; must be DNS-compliant (no underscores, no adjacent dots, no IP-address-form names). A sane enterprise convention: `<company>-<env>-<purpose>-<region>` e.g. `cymbank-prod-txnlake-eu-west-1`.
...
```
s3://cymbank-prod-txnlake/2026/08/17/trades/part-00001.parquet
```
...
|---|---|---|
| Bucket | Global-namespace container; unit of config, policy, billing | `cymbank-prod-txnlake` |
...
| IAM policy | Identity-side permissions | `s3:GetObject` on `cymbank-prod-*/reports/*` for the reporting role |
...
- Directory buckets use a **regional namespace** (the one exception to the global bucket-name namespace) and a distinct naming scheme (`<bucket-name>--<azid>--x-s3`, e.g., `cymbank-ml-data--use1-az4--x-s3`).
...
|---|---|---|---|
| `cymbank-prod-txnlake` | Trade and transaction lake | `raw/`, `curated/`, `consumption/`, `audit/` | Standard → IA → Glacier by age |
| `cymbank-prod-backup` | System + DB backups | `rds/`, `ebs/`, `app/` | IA → Glacier; CRR to EU |
| `cymbank-prod-archive` | Regulatory WORM archive | `trade-records/`, `statements/`, `evidence/` | Glacier Flexible/Deep Archive + Object Lock |
| `cymbank-prod-artifacts` | Builds, reports, shared files | `ci/`, `reports/` | Standard-IA; presigned URLs for distribution |
| `cymbank-ml-data--use1-az4--x-s3` (directory bucket) | Feature-store staging, training loads | `features/`, `checkpoints/` | Express One Zone (regenerate-able) |
...
- **SRR compliance copy**: `txnlake` replicates to a locked `cymbank-prod-txnlake-compliance` bucket (different account) for the auditors; **CRR** to `eu-west-1` for the European entities' data-residency copy.
